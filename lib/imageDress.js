...
const LEGACY_DIR = path.join(__dirname, "..", "public", "generated");
const OWNERS_FILE = path.join(OUT_DIR, "owners.json");
const GEN_NAME_RE = /^[df][a-f0-9]{16,}\.(jpg|jpeg|png|webp)$/i;
...
function bootGeneratedStore() {
  ensureOutDir();
  migrateLegacyGenerated();
  mergeBillingOwners();
  billingOwnersMerged = true;
}

function migrateLegacyGenerated() {
  if (!fs.existsSync(LEGACY_DIR)) return;
  let names = [];
  try {
    names = fs.readdirSync(LEGACY_DIR);
  } catch (_) {
    return;
  }
  names.forEach(function (name) {
    if (!name || name.startsWith(".") || name === "owners.json") return;
...
let billingOwnersMerged = false;

function loadOwners() {
  if (ownersCache) return ownersCache;
  try {
    ownersCache = JSON.parse(fs.readFileSync(OWNERS_FILE, "utf8")) || {};
  } catch (_) {
    ownersCache = {};
  }
  return ownersCache;
...
function saveOwners() {
  ensureOutDir();
  fs.writeFileSync(OWNERS_FILE, JSON.stringify(loadOwners()));
}

function rememberOwner(name, userId) {
  if (!GEN_NAME_RE.test(name) || !userId) return;
  loadOwners()[name] = String(userId);
  saveOwners();
}
...
function mergeBillingOwners() {
  const map = loadOwners();
  const fromDb = billing.listPhotoFileOwners();
  Object.keys(fromDb).forEach(function (name) {
    if (!map[name]) map[name] = fromDb[name];
  });
  saveOwners();
}

function isGeneratedName(name) {
  return GEN_NAME_RE.test(String(name || ""));
}

function generatedFilePath(name) {
  if (!isGeneratedName(name)) return "";
  const root = path.resolve(OUT_DIR);
  const full = path.resolve(path.join(OUT_DIR, path.basename(name)));
  if (full !== root && !full.startsWith(root + path.sep)) return "";
  return full;
}

function userOwnsGenerated(userId, name) {
  const id = String(userId || "");
  const file = path.basename(String(name || ""));
  if (!id || !isGeneratedName(file)) return false;
  const map = loadOwners();
  if (map[file] === id) return true;
  if (map[file] && map[file] !== id) return false;
  if (!billingOwnersMerged) {
    mergeBillingOwners();
    billingOwnersMerged = true;
  }
  return loadOwners()[file] === id;
}
...
function sweepOldFiles() {
  ...
  try {
    for (const name of fs.readdirSync(OUT_DIR)) {
      if (name.startsWith(".") || name === "owners.json") continue;
...
async function understandPhotoInstruction({
  ...
  if (comfyQwen.comfyEnabled() && String(process.env.IMAGE_BACKEND || "auto").toLowerCase() !== "venice") {
    return raw;
  }
...
function buildComfyPrompt({
  ...
  const wantsNude = /\b(nude|naked|nangi|nanga)\b/i.test(meaning || said);
  const lines = [];
  ...
  if (wantsNude) {
    lines.push(
      "CLOTHES: completely nude unclothed adult woman. Breasts and body fully visible. No blouse, no fabric covering the chest. Do not keep the original outfit."
    );
  } else if (outfit && outfit.id && outfit.id !== "keep") {
...
async function editPhoto({
  ...
  const tryVenice =
    mode === "venice" || (mode === "auto" && !comfyQwen.comfyEnabled());
  let lastErr = "";
...
function saveGenerated(buf, mime, prefix, userId) {
  ensureOutDir();
  sweepOldFiles();
...
async function dressPhoto({
  ...
  if (!identityB64 && identityImage) {
    try {
      ...
      savedIdentityUrl = saveGenerated(decoded.buf, decoded.mime, "f", userId);
    } catch (_) {
      identityB64 = "";
    }
  }
  ...
  const painted = await editPhoto({
    ...
    iterate,
    skipLora: isSexyPrompt(extraRaw || extra),
  });
  const outBuf = painted.buf;
  const url = saveGenerated(outBuf, "image/jpeg", "d", userId);
