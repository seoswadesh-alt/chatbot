...
function buildComfyPrompt({
  ...
  const lines = [];
  if (meaning) {
    lines.push("Edit this photograph to show this scene:");
    lines.push(meaning);
  ...
  lines.push(
    "Keep her exact face, hair, age, skin tone, and body size. Do not swap identity."
  );
  lines.push(
  ...
  if (isSexyPrompt(meaning || said)) {
    lines.push("Erotic 18+. The requested action must be visible. Do not modest-wash.");
  }
