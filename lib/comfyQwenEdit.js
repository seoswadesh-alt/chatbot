...
function pickOnDisk(envName, names, patterns, fallbackFirst) {
  const list = Array.isArray(names) ? names : [];
  const wanted = String(envName || "").trim();
  if (wanted && list.indexOf(wanted) !== -1) return wanted;
  return pickName(list, patterns, fallbackFirst);
}
...
async function probeComfy() {
  ...
  try {
    ...
    const unetName = pickOnDisk(
      process.env.COMFY_UNET,
      unet,
      [/qwen.*edit/i, /qwen_image_edit/i],
      true
    );
    const clipName = pickOnDisk(
      process.env.COMFY_CLIP,
      clip,
      [/uncensored|heretic/i, /qwen_2\.5_vl/i, /qwen2\.5.vl/i, /qwen.*vl/i],
      true
    );
    const vaeName = pickOnDisk(
      process.env.COMFY_VAE,
      vae,
      [/qwen_image_vae/i, /qwen.*vae/i],
      true
    );
    const loraName = pickOnDisk(
      process.env.COMFY_LORA,
      lora,
      [/lightning/i, /qwen-image-edit.*4\s*step/i, /4steps/i],
      false
    );
    const ready = !!(unetName && clipName && vaeName) || !!loadSavedWorkflow();
    const uncensoredClip = /uncensored|heretic/i.test(clipName);
    return {
      ok: ready,
      configured: true,
      url,
      ...
      uncensoredClip: uncensoredClip,
      clipHint: uncensoredClip
        ? ""
        : "Official CLIP is on disk (keeps clothes). Upload qwen_2.5_vl_7b_uncensored_comfy_ready_bf16.safetensors into ComfyUI/models/text_encoders via FileBrowser (8080).",
      error: ready
        ? ""
...
async function editWithComfy({ imageB64, prompt, skipLora }) {
  ...
  const lora = skipLora ? "" : health.models.lora;
  const workflow = saved
    ? injectSavedWorkflow(saved, imageName, prompt)
    : buildWorkflow({
        unet: health.models.unet,
        clip: health.models.clip,
        vae: health.models.vae,
        lora: lora,
        imageName: imageName,
        prompt: prompt,
